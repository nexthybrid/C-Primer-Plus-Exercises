...
int taxCalc(float grossPay)
{
	if (grossPay <= TAX_LADDER_1) {
		return TAX_RATE_1 * grossPay;
	} else if (grossPay <= TAX_LADDER_1 + TAX_LADDER_2){
		return TAX_RATE_1 * TAX_LADDER_1 + TAX_RATE_2 * (grossPay - TAX_LADDER_1);
	} else {
		return TAX_RATE_1 * TAX_LADDER_1 + TAX_RATE_2 * TAX_LADDER_2 
			+ TAX_RATE_3 * (grossPay  - TAX_LADDER_1 - TAX_LADDER_2);
	}
}

void printPay(float hrRate)
{
	printf("Enter hours worked in a week:\n");
	int hours;
	scanf("%d", &hours);
	printf("Hours worked:\t\t%d\n", hours);
	printf("Base pay rate: $%.2f/hr\n", hrRate);
	int overHours = (hours > OVERTIME_HR ? hours - OVERTIME_HR : 0);
	printf("Overtime:\t\t%d\n", overHours);
	float grossPay = (hours > OVERTIME_HR ? 
		(OVERTIME_FACTOR * overHours + OVERTIME_HR) * hrRate : hours * hrRate);
	printf("Gross pay:\t\t%.2f\n", grossPay);
	// tax calculation
	int tax = taxCalc(grossPay);
	printf("Total tax:\t\t%.2f\n", tax);
	printf("\nNet pay:\t\t%.2f\n\n", grossPay - tax);
}
